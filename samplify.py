        ...
        if closest_label:
            ...
            if closest_label:
                ...
                if label == 0:
                    ...
                    feature["Label"] = "Partially Collapsed"
                elif label == 2:
                    feature["Label"] = "Fully Collapsed"
                else:
                    raise ValueError(f"Invalid label {label} encountered. Only labels 0, 1, or 2 are allowed.")
...
def build_categorical_summary(
    ...
    all images together and appends it as a new sheet to the summary Excel.

    Parameters
    ----------
    feature_csv_path : str
        Path to the combined feature CSV (contains all images).
    excel_summary_path : str
        Path to the Excel file where summary sheets are saved.
    sheet_name : str
        Name of the new sheet for categorical summary.
    feature_columns : list[str] or None
        Feature columns to include. If None, they are inferred automatically.
    ...
    required_cols = {"Image Name", "Predicted Label"}
    if not required_cols.issubset(df.columns):
        print("ERROR: CSV must contain 'Image Name' and 'Predicted Label' columns.")
        return None

    # Remove leftover index column if present
    ...
    # Group by image → then category → compute means
    grouped = (
        df.groupby(["Image Name", "Predicted Label"])[feature_columns]
        .mean()
        .reset_index()
    )
    ...
    try:
        with pd.ExcelWriter(excel_summary_path,
                            engine="openpyxl",
                            mode="a",
                            if_sheet_exists="replace") as writer:
            categorical_summary_df.to_excel(
                writer,
                sheet_name=sheet_name,
                index=False
            )
        print(categorical_summary_df.head())
